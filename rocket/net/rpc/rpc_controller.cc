...
namespace rocket{
    void RpcController::Reset()  //用于下一次rpc调用时的重置
    {
        m_error_code = 0;
        m_error_info = "";
        m_msg_id = "";
        ...
        m_timeout = 1000; //ms 超时时间
    }
    bool RpcController::Failed() const  //获取当前rpc调用是否成功
    {
        return m_is_failed;
    }
    
    std::string RpcController::ErrorText() const{  //返回错误信息
        return m_error_info;
    }
